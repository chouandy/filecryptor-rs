...
use crate::{filecrypt::encrypt_file, secrets::get_secrets_password_from_parameter_store};
...
pub struct Command {
    ...
    /// The parameter store name for encrypt
    #[arg(
        long,
    ...
    /// The parameter store region for encrypt
    #[arg(
        long,
...
impl Command {
    pub async fn execute(&self) -> Result<(), Box<dyn Error>> {
        ...
        let dst = format!("{}.enc", src);

        print!("Encrypting file...");
        encrypt_file(&src, &dst, password.as_bytes())?;
        println!("done");
